...
Please create a file called config.js at the root of the target directory
    ...
    await Promise.all(roots.map(async root => {
      if (root.hasParentInConfig()) {
        const parentRoot = roots.find(_root => {
          return _root.name === root.config.parent;
        });
        root.outputRoot = parentRoot.name;

        const _url = (() => {
          const urlObject = url.parse(parentRoot.config.url);
          urlObject.pathname = path.join(urlObject.pathname, root.name);
          return url.format(urlObject);
        })();
        root.config = Object.assign({
          parent: root.config.parent
        }, parentRoot.config, root.config.override, {url: _url});
        root.postProcess();
      } else {
        root.outputRoot = root.name;
      }
        ...
        dir.posts.map(post => {
          ...
          if (dir.isRenders()) {
            ...
            Object.assign(opts, {
              path: path.join(
                root.outputRoot,
                post.absURL.slice(1) + 'index.html'),
              contents: Buffer.from(html)
            });
          } else {
            Object.assign(opts, {
              path: path.join(root.outputRoot, post.absURL.slice(1)),
  ...
  async bundle() {
    ...
    const promises = this.roots
      .filter(root => !root.hasParentInConfig())
      .map(async root => {
        ...
        _webpackConfig.resolve.modules = [
          ...
          path.join(__dirname, '../node_modules/'),  // of this
          path.join(__dirname, '../..'),             // When installed side
          root.config.theme.Renderer.nodeModulesPath // for yarn link
        ];
